...
SMC_MIN_SCORE = float(os.getenv('SMC_MIN_SCORE', '60'))
ICT_MIN_SCORE = float(os.getenv('ICT_MIN_SCORE', '60'))
WYCKOFF_MIN_SCORE = float(os.getenv('WYCKOFF_MIN_SCORE', '60'))
...
def _resolve_trade_levels(best, ai):
    """Once a candidate has passed the >=2/3 strategy vote, quant has
    proposed a trade. SL/TP are NEVER generated by Gemini -- they always
    come from quant's own structure/volatility-based computation (already
    in `best`, from build_analysis). Gemini's role is independent
    setup-quality filtering: APPROVE with score>=GEMINI_MIN_SCORE is
    required for the trade to actually go out; a REJECT or a low-confidence
    APPROVE now vetoes it instead of just being logged as a note.
    Returns (trade_plan, note, gemini_score, approved).
    """
    try:
        gemini_score=float(ai.get('score',ai.get('confidence',0)) or 0)
    except (TypeError,ValueError):
    ...
    approved=gemini_decision=='APPROVE' and gemini_score>=GEMINI_MIN_SCORE
    if approved:
        ...
        note=f"Gemini did not confirm ({why}, score={gemini_score:.0f}) -- trade rejected"

    gemini_reason=ai.get('reason','').strip()
    full_note=f'[Gemini: {gemini_reason}] {note}' if gemini_reason else note
    return trade_plan, full_note, gemini_score, approved
...
def _score_smc(a1,a2,a3,c1,c2,c3,d):
    ...
    sum to 100: htf_bias 20, structure/liquidity(trend) 15, POI 15,
    sweep 20, BOS 15, CHOCH 5, CRT/TBS 5, momentum+volume(entry confirm) 5.
    Reweighted 2026-09 from research on ICT/SMC order-block validity
    criteria (liquidity sweep is the primary filter -- "no sweep before
    the OB = skip the trade" is repeatedly cited as eliminating most
    losing OB setups) -- sweep raised 15->20, POI (raw order-block/FVG
    location alone, without sweep+displacement+BOS context, is weak
    evidence per the same sources) lowered 20->15. UNVALIDATED against
    this bot's own trade history -- revisit with factor_backtest_report
    once there is enough closed-trade data."""
    ...
    score=(20*htf_strength+15*trend_strength+15*poi_strength+20*sweep_strength
...
def _score_ict(a1,a2,a3,c1,c2,c3,d):
    ...
    retracement entry. Weights sum to 100: liquidity_1h 20, sweep_15m 25,
    displacement_15m 25, fvg_15m 15, killzone_5m 5, ote_5m 10.
    Reweighted 2026-09: research consistently treats liquidity sweep and
    displacement as the two hard structural filters ("no liquidity sweep
    before the OB = skip the trade -- this single filter eliminates the
    majority of losing order block setups"; weak/grinding moves without
    displacement "reduce edge"), so both raised 20->25. Killzone timing
    and OTE are entry-precision/session filters, not evidence the setup
    itself is real, so both lowered (killzone 10->5, OTE 15->10).
    UNVALIDATED against this bot's own trade history -- revisit with
    factor_backtest_report once there is enough closed-trade data."""
    ...
    score=(20*liq_strength+25*sweep_strength+25*disp_strength+15*fvg_strength
           +5*killzone_strength+10*ote_strength)
    reasons=[]
    ...
    try:
        ...
        for rank,best in enumerate(candidates[:MAX_GEMINI_CANDIDATES],1):
            ...
            try:
                ...
                trade_plan.update({'rr':abs(best['tp2']-best['entry'])/max(abs(best['entry']-best['sl']),1e-12)})
                gemini_score=0.0
                approved=True  # Gemini couldn't be reached at all -- fall back to quant's vote
            else:
                trade_plan,gemini_note,gemini_score,approved=_resolve_trade_levels(best,ai)

            if approved:
                ok,level_reason=_final_level_check(
                    trade_plan['direction'],trade_plan['entry'],trade_plan['sl'],
                    trade_plan['tp1'],trade_plan['tp2'],trade_plan['tp3']
                )
                if not ok:
                    approved=False
                    gemini_note=f'{gemini_note} | Level check failed: {level_reason} -- trade rejected'
